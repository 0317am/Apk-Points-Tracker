...
function ensureFullRoster(){
  // migrate older saves up to the original 66 roster.
  // Any accounts added beyond 66 (via "+ Add Account") are left untouched.
...
document.getElementById("resetBtn").addEventListener("click", async () => {
  if(!confirm(`Reset all ${rows.length} accounts to zero? This clears points, targets and bonuses. Accounts you've added beyond the original 66 will be removed.`)) return;
  rows = defaultRows();
  await saveRows();
  render();
});
