...
function ensureFullRoster(){
  // No longer auto-fills a default roster — the app starts blank.
  // Existing accounts (yours, or migrated from an older save/import) are
  // left exactly as they are; just keep them sorted by id.
...
function applyPresetNames(){
  // Disabled — app starts blank, no preset account names/servers are applied.
  return;
}
...
function addNewAccount(){
  ...
  saveRows();
  render();
  showToast("Account " + nextId + " added");
  expanded.add(nextId);
  render();
  const card = document.getElementById("card-" + nextId);
...
async function loadData(){
  ...
  try{
    ...
    if(s){ dailyPoints = JSON.parse(s).dailyPoints || 170; }
  }catch(e){ /* use default */ }
  try{
    const l = await storageGet(REDEEM_LOG_KEY);
    redeemLog = l ? JSON.parse(l) : [];
  }catch(e){ redeemLog = []; }
  ...
  try{
    const sp = await storageGet(SELL_PRICES_KEY);
    if(sp){
      const parsed = JSON.parse(sp);
      // migrate old flat "5"/"10" keys (pre currency-split) into both EUR/USD variants
  ...
  try{
    const g = await storageGet(GOAL_KEY);
    if(g){
      const parsed = JSON.parse(g);
      goalAmount = parsed.amount || null;
      goalDate = parsed.date || null;
    }
  }catch(e){ /* keep defaults */ }
...
async function saveSellPrices(){
  try{ await storageSet(SELL_PRICES_KEY, JSON.stringify(sellPrices)); }catch(e){}
}

async function saveGoal(){
  try{ await storageSet(GOAL_KEY, JSON.stringify({ amount: goalAmount, date: goalDate })); }catch(e){}
}

async function saveRows(){
  try{ await storageSet(STORAGE_KEY, JSON.stringify(rows)); }catch(e){}
}
async function saveSettings(){
  try{ await storageSet(SETTINGS_KEY, JSON.stringify({dailyPoints})); }catch(e){}
}
async function saveRedeemLog(){
  try{ await storageSet(REDEEM_LOG_KEY, JSON.stringify(redeemLog)); }catch(e){}
}
  ...
  display.forEach(({row, calc}) => {
    ...
    card.innerHTML = `
      ...
      <div class="details ${isOpen ? '' : 'hidden'}">
        ...
        <button class="redeem-btn" type="button" style="background:transparent;border:1px solid var(--danger);color:var(--danger);" data-id="${row.id}" data-action="deleteAccount">Delete Account</button>
        <div class="row-grid">
...
document.getElementById("redeemLogPanel").addEventListener("change", async (e) => {
  ...
  if(action === "uploadProof" && entryId){
    ...
    if(!entry) return;
    try{
      entry.proof = await fileToDataUrl(file);
      await saveRedeemLog();
      renderRedeemLog();
    }catch(err){
...
document.getElementById("redeemLogPanel").addEventListener("click", (e) => {
  ...
  if(action === "downloadProof" && entry.proof){
    ...
    const filename = `proof-${entry.id}-${entry.date}.${ext}`;
    const blob = dataUrlToFile(entry.proof, filename);
    nativeDownload(blob, filename, blob.type);
  }
...
document.getElementById("resetBtn").addEventListener("click", async () => {
  if(!confirm(`Delete all ${rows.length} account${rows.length === 1 ? '' : 's'}? This clears everything — points, targets, and bonuses.`)) return;
  rows = defaultRows();
  await saveRows();
  render();
});
...
// Saves/shares a file reliably from inside the packaged Android app.
// Order of attempts:
// 1. Capacitor's Filesystem + Share plugins (the officially supported way —
//    writes the file to app cache storage, then opens the native share sheet
//    so you can save it to Downloads/Drive or send it directly).
// 2. Web Share API with files (works in some newer WebView versions).
// 3. Old-style blob-anchor download (works in regular desktop/mobile browsers).
async function nativeDownload(content, filename, type){
  try{
    ...
    if(cap && cap.Plugins && cap.Plugins.Filesystem && cap.Plugins.Share){
      ...
      await cap.Plugins.Share.share({ title: filename, url: writeResult.uri });
      return;
    }
...
document.getElementById("exportJsonBtn").addEventListener("click", () => {
  ...
  nativeDownload(payload, "points-tracker-" + todayStr() + ".json", "application/json");
});
